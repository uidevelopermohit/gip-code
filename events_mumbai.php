<?php include('includes/header.php')?>
<div class="inner_page_wrapper">
    <!--Inner Banner Start-->
    <Section class="inner_banner ">
        <div class="container_fluid">
            <div class="flex_row">
                <div class="col_8">
                    <h4>News & Events</h4>
                </div>

                <div class="col_4">
                    <ul class="new_breadcrumb">
                        <li><a href="<?php echo $base_url; ?>">Home</a></li>
                        <li>News & Events</li>
                    </ul>
                </div>
            </div>
        </div>
    </Section>
    <!--Inner Banner End-->
    <!--First Fold Section Start-->
    <section class="events_detail gradient_container common_spacing">
        <div class="container_fluid">
            <div class="flex_row mb_60">
                <div class="col_4 event_detail_desc">
                    <h6>3rd ECSWG meet: Mumbai </h6>
                    <p>(21 to 23 May 2023)</p>

                </div>
                <div class="col_8">
                    <img src="assets/images/events/3rd_EWCSG.webp" alt="">
                </div>
            </div>
            <div class="flex_row">
                <div class="col_12">
                    <p>
                        <strong>
                            G20's Third Environment and Climate Sustainability Working Group (ECSWG) Meeting was
                            held in Mumbai from May 21-23,
                            2023
                        </strong>
                    </p>
                    <p>
                        The 3rd Environment and Climate Sustainability Working Group (ECSWG) meeting in Mumbai will
                        focus on
                        outcomes-driven discussions, including Beach Clean Up and the Ocean 20 Dialogue. The meeting
                        aims to
                        enhance cooperation among G20 nations for sustainable ocean management and marine
                        biodiversity conservation.
                        The sessions will cover science, technology, innovation, policy, governance, and
                        participation, as well as
                        establishing blue finance mechanisms. The Ministry of Environment, Forest and Climate Change
                        is committed
                        to working with all stakeholders to achieve a sustainable and resilient future.
                    </p>
                    <!--pdf Section Start-->
                    <div class="pdf_document">
                        <a href="assets/pdf/G20_Third_ECSWG_Meeting_to_be_held_in_Mumbai_from_May_21-23_2023.pdf"
                            target="_blank">
                            <img src="assets/images/pdf_icon.webp" class="pdf_icon">
                            <span>G20 Third ECSWG Meeting to be held in Mumbai from May 21-23-2023.pdf</span>
                        </a>
                    </div>
                    <!--pdf Section End-->
                </div>
            </div>
        </div>
    </section>


    <!--Event Section Start-->
    <section class="events_section common_spacing">
        <div class="container_fluid">
            <div class="section_heading">
                <h2>Events & News</h2>
            </div>

            <div class="owl-carousel events_slider">
                <div class="events_card">
                    <div class="events_card_image">
                        <img src="assets/images/events/2nd_ECWSG.webp" alt="">
                    </div>
                    <div class="events_card_content">
                        <a href="events_newdelhi.php" style="color:#000;">
                            <h5>New Delhi Leaders’ Declaration (New Delhi)</h5>
                            <p>9th September 2023</p>
                        </a>
                    </div>
                </div>
                <div class="events_card">
                    <div class="events_card_image">
                        <img src="assets/images/events/ECSWG1.webp" alt="">
                    </div>
                    <div class="events_card_content">
                        <a href="events_bengaluru.php" style="color:#000;">
                            <h5>1st ECSWG meet: Bengaluru</h5>
                            <p>February 9 to 11, 2023</p>
                        </a>
                    </div>
                </div>
                <div class="events_card">
                    <div class="events_card_image">
                        <img src="assets/images/events/2nd_ECWSG.webp" alt="">
                    </div>
                    <div class="events_card_content">
                        <a href="events_gandhinagar.php" style="color:#000;">
                            <h5>2nd ECSWG meet: Gandhinagar</h5>
                            <p>March 27 to 29, 2023</p>
                        </a>
                    </div>
                </div>
                <div class="events_card">
                    <div class="events_card_image">
                        <img src="assets/images/events/3rd_EWCSG.webp" alt="">
                    </div>
                    <div class="events_card_content">
                        <a href="events_mumbai.php" style="color:#000;">
                            <h5>3rd ECSWG meet: Mumbai</h5>
                            <p>May 21 to 23, 2023</p>
                        </a>
                    </div>
                </div>
                <div class="events_card">
                    <div class="events_card_image">
                        <img src="assets/images/events/ECSWG_Chennai.webp" alt="">
                    </div>
                    <div class="events_card_content">
                        <a href="events_chennai.php" style="color:#000;">
                            <h5>4th ECSWG meet: Chennai</h5>
                            <p>July 26 to 28, 2023</p>
                        </a>
                    </div>
                </div>
            </div>
        </div>
    </section>
    <!--Event Section End-->
</div>
<?php include('includes/footer.php')?>